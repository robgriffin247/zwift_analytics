...
google_document_id = os.getenv("GS__ZWIFT_ANALYTICS__SERIES_ADMIN")
series_sheet_id = os.getenv("GS__ZWIFT_ANALYTICS__SERIES_ADMIN__SERIES")
...
def generate_riders_seed():
    ...
    with duckdb.connect() as con:
        ...
        active_series = con.sql(f"""
            select * 
            from read_csv_auto('https://docs.google.com/spreadsheets/d/{google_document_id}/export?format=csv&gid={series_sheet_id}')
            where active_from_date<=now() and active_to_date>=now() and riders_sheet_id is not null
            """).pl()
        ...
        for series in active_series.iter_rows(named=True):
            ...
            riders += [series_events]
    
    pl.concat(riders).write_csv("seeds/riders.csv")

    return f"{_logging_time()}: Riders seed generated"

if __name__=="__main__":
    print(generate_events_seed())
    print(generate_riders_seed())